...
angular.module('myApp.view1', ['ngRoute'])
	...
	.controller('View1Ctrl', ['filterFilter', 'orderByFilter', '$scope', function (filterFilter, orderByFilter, $scope) {
		...
		$scope.pokemonsFiltred = $scope.pokemons;

        $scope.myFilter = function(){
            let filteredByQuery = filterFilter($scope.pokemons, $scope.myQuery);
            let filteredByType = filterFilter(filteredByQuery, { type: $scope.myType });
            $scope.pokemonsFiltred = orderByFilter(filteredByType, $scope.myOrderProperty);
        }
